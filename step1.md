...
```
curl https://dlcdn.apache.org/cassandra/4.0.13/apache-cassandra-4.0.13-bin.tar.gz \
        --output apache-cassandra-4.0.13-bin.tar.gz
```
...
✅ Extract tarball:
```
tar xf apache-cassandra-4.0.13-bin.tar.gz
```

The directory should now contain the tarball and the `apache-cassandra-4.0.13` directory
